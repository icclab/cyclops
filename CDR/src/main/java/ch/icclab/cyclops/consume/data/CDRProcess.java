 ...
 */
public class CDRProcess implements Runnable {
    ...
    public CDRProcess(String content) {
        this.content = content;
    }

    public CDRProcess(String content, ConsumerEntry consumer, Long deliveryTag, boolean healthCheck) {
        this.content = content;
     ...
     * @param list of CDR data
     * @return status
     */
    private DbAccess.PersistenceStatus persistList(List<CDR> list) {
     ...
     * @return list or null
     */
    private List<CDR> parseWhatCouldBeJSONList(String json) {
        try {
            Type CDRType = new TypeToken<List<CDR>>(){}.getType();
            return getNewGson().fromJson(json, CDRType);
        } catch (Exception e) {
            return null;
        }
    }
     ...
     * @return list or null
     */
    private List<CDR> parseWhatCouldBeJSONObject(String json) {
        try {
            CDR parsed = getNewGson().fromJson(json, CDR.class);
    ...
    /**
     * Get new Gson instance with registered processor that handles CDR object
     * @return Gson
     */
    private Gson getNewGson() {
        return new GsonFireBuilder().registerPreProcessor(CDR.class, new CDRDeserializer<>()).createGson();
    }
