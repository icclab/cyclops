...
class CommandMapping {
                ...
                public Class<? extends Command> getClassForElement(JsonElement jsonElement) {
                    try {
                        ...
                        list.addAll(new ClassesInPackageScanner().setResourceNameFilter((packageName, fileName)
                                -> clazz.equalsIgnoreCase(RegexParser.getFileName(fileName))).scan(CommandMapping.class.getPackage().getName()));
    ...
    static Command fromJson(String json) {
        try {
            ...
            return (obj != null && !obj.getClass().equals(Command.class))? obj : null;
        } catch (Exception e) {
            return null;
        }
    }
