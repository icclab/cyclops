...
public class Settings {
    ...
    protected GitCredentials gitCredentials;
    protected RollbackEndpoints rollbackEndpoints;
     ...
     * @return credentials
     */
    private GitCredentials loadGitCredentials() {
        GitCredentials gitCredentials = new GitCredentials();

        gitCredentials.setGitRepo(properties.getProperty("GitRepo"));
        gitCredentials.setGitUsername(properties.getProperty("GitUsername"));
        gitCredentials.setGitPassword(properties.getProperty("GitPassword"));
        gitCredentials.setGitProjectPath(properties.getProperty("GitProjectPath"));
    ...
    public GitCredentials getGitCredentials() {
        ...
        if (gitCredentials == null) {
            try {
                gitCredentials = loadGitCredentials();
            } catch (Exception e) {
                logger.error("Could not load Git credentials from configuration file: " + e.getMessage());
            }
        }
    ...
    //=============== Rollback Endpoints
    /**
     * Load Rollback Endpoints
     * @return endpoints
     */
    private RollbackEndpoints loadRollbackEndpoints() {
        RollbackEndpoints rollbackEndpoints = new RollbackEndpoints();

        rollbackEndpoints.setUdrendpoint(properties.getProperty("UDREndpoint"));
        rollbackEndpoints.setCdrendpoint(properties.getProperty("CDREndpoint"));
        rollbackEndpoints.setBillingendpoint(properties.getProperty("BillingEndpoint"));


        return rollbackEndpoints;
    }

    /**
     * Access loaded Rollback Endpoints
     * @return cached endpoints
     */
    public RollbackEndpoints getRollbackEndpoints() {

        if (rollbackEndpoints == null) {
            try {
                rollbackEndpoints = loadRollbackEndpoints();
            } catch (Exception e) {
                logger.error("Could not load Rollback Endpoints from configuration file: " + e.getMessage());
            }
        }
