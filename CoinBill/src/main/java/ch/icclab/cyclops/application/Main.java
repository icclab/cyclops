...
import ch.icclab.cyclops.rule.versioning.GitConnector;
 ...
 * Created: 26/04/16
 * Description: Entry point for Rule engine micro service
 */
public class Main extends Application{
    ...
    private static final int ERR_RABBITMQ = 6;
    private static final int ERR_ROUTER = 7;
    public static final int ERR_HEALTH = 8;
    private static final int ERR_START = 9;
    ...
    private static final String helpMessage = String.join(System.getProperty("line.separator"),
            ...
            "Author: Martin Skoviera, Cyclops Labs GmbH", "",
            "Required parameters: path to configuration file",
            "Optional parameters: HTTP port","",
            "Example: java -jar coin.jar coincdr.conf 4570");

    public static void main(String[] args){
        outputProgressBar("Loading RCB Cyclops Rule engine micro service ");
    ...
    private static void checkParameters(String[] args) {
        if (args.length == 0) {
            String log = "A configuration file path has to be provided (as argument), otherwise Rule engine micro service cannot be properly loaded";
            logger.error(log);
    ...
    private static void checkAndConfigureHibernate() {
        try {
            ...
            // create Hibernate
            HibernateClient client = HibernateClient.createInstance(configuration);

            // check the connection
            client.ping();

        } catch (Exception e) {
            String log = String.format("Hibernate: %s", e.getMessage());
            logger.error(log);
            System.err.println(log);
    ...
    private static void checkAndStartServer(Component component) {
        logger.trace("Starting the Rule engine micro service");
        try {
