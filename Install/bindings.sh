...
while : ; do
  ...
  then
    ...
    echo "Declaring binding between cyclops.udr.dispatch and cyclops.udr.commands with routing key SelfPublish on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d "{\"routing_key\":\"SelfPublish\"}" http://localhost:15672/api/bindings/%2F/e/cyclops.udr.dispatch/q/cyclops.udr.commands >/dev/null 2>&1
...
while : ; do
  ...
  then
    ...
    echo "Declaring binding between cyclops.coincdr.dispatch and cyclops.billing.commands with routing key Billing on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"Billing\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coincdr.dispatch/q/cyclops.billing.commands	>/dev/null 2>&1

    echo "Declaring binding between cyclops.coincdr.dispatch and cyclops.cdr.commands with routing key CDR on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"CDR\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coincdr.dispatch/q/cyclops.cdr.commands	>/dev/null 2>&1

    echo "Declaring binding between cyclops.coincdr.dispatch and cyclops.udr.commands with routing key UDR on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"UDR\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coincdr.dispatch/q/cyclops.udr.commands	>/dev/null 2>&1
    break
  else
    echo "Waiting for RabbitMQ (publisher host localhost:15672) service to become active"
    sleep 5
...
while : ; do
  ...
  then
    ...
    echo "Declaring binding between cyclops.coinbill.dispatch and cyclops.billing.commands with routing key Billing on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"Billing\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coinbill.dispatch/q/cyclops.billing.commands	>/dev/null 2>&1

    echo "Declaring binding between cyclops.coinbill.dispatch and cyclops.cdr.commands with routing key CDR on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"CDR\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coinbill.dispatch/q/cyclops.cdr.commands	>/dev/null 2>&1

    echo "Declaring binding between cyclops.coinbill.dispatch and cyclops.udr.commands with routing key UDR on localhost:15672 ... "
    curl -u "guest:guest" -H "content-type:application/json" -XPOST -d '{\"routing_key\":\"UDR\"}' http://localhost:15672/api/bindings/%2F/e/cyclops.coinbill.dispatch/q/cyclops.udr.commands	>/dev/null 2>&1
    break
  else
    echo "Waiting for RabbitMQ (publisher host localhost:15672) service to become active"
    sleep 5
