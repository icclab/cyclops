 ...
 * Created: 12.05.17
 * Description: Handle edge cases for Usage JSON deserialization
 */
public class UsageDeserializer<T> implements PreProcessor <T> {
    @Override
    public void preDeserialize(Class<? extends T> clazz, JsonElement jsonElement, Gson gson) {
        ...
        if (jsonElement != null && jsonElement.isJsonObject()) {
            ...
            if (root.has(Usage.DATA_FIELD.getName())) {
                root.addProperty(Usage.DATA_FIELD.getName(), new Gson().toJson(root.get(Usage.DATA_FIELD.getName())));
            }
        }
    }
}
