 ...
 */
public class UsageProcess implements Runnable {
    ...
    public UsageProcess(String content) {
        this.content = content;
    }

    public UsageProcess(String content, ConsumerEntry consumer, Long deliveryTag, boolean healthCheck) {
        this.content = content;
    ...
    @Override
    public void run() {
        // parse JSON into list of Usage objects
        List<Usage> parsed = parseWhatCouldBeJSONObject(content);
     ...
     * @param list of usage data
     * @return status
     */
    private DbAccess.PersistenceStatus persistList(List<Usage> list) {
     ...
     * @return list or null
     */
    private List<Usage> parseWhatCouldBeJSONList(String json) {
        try {
            Type usageType = new TypeToken<List<Usage>>(){}.getType();
            return getNewGson().fromJson(json, usageType);
        } catch (Exception e) {
            return null;
        }
    }
     ...
     * @return list or null
     */
    private List<Usage> parseWhatCouldBeJSONObject(String json) {
        try {
            Usage parsed = getNewGson().fromJson(json, Usage.class);
    ...
    /**
     * Get new Gson instance with registered processor that handles Usage object
     * @return Gson
     */
    private Gson getNewGson() {
        return new GsonFireBuilder().registerPreProcessor(Usage.class, new UsageDeserializer<>()).createGson();
    }
