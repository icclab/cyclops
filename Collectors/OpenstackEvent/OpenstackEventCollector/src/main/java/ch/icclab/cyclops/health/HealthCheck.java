...
public class HealthCheck implements Runnable {
    ...
    public void run() {
        try {
            // check the database connection whether it is alive
            if (new DbAccess().ping()) logger.trace("Health check: Database connection is alive");
            else throw new Exception("Database is down");
