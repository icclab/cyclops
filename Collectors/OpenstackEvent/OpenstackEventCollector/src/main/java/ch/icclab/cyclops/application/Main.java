...
import ch.icclab.cyclops.consume.command.RabbitMQStarter;
...
public class Main extends Application{
    ...
    private static final String helpMessage = String.join(System.getProperty("line.separator"),
            "RCB Cyclops: Cyclops OpenStack Event collector",
            "Author: Martin Skoviera, cyclops-labs.io", "",
            "Required parameters: path to configuration file",
            "Optional parameters: HTTP port","",
            "Example: java -jar cyclops_event_collector.jar config.txt 4567");

    public static void main(String[] args){
        outputProgressBar("Loading RCB Cyclops OpenStack Event collector:");
        ...
        // check and setup RabbitMQ publisher and process
    ...
    private static void checkParameters(String[] args) {
        if (args.length == 0) {
            String log = "A configuration file path has to be provided (as argument), otherwise Event collector cannot be properly loaded";
            logger.error(log);
    ...
    private static void checkAndSetupRabbitMQ() {
        ...
        if(Loader.getSettings().getOpenstackSettings().getEstablishBindings()){

            RabbitMQStarter starter = new RabbitMQStarter();
            starter.checkAndDeclareBindings();

            if (starter.getStatus().hadClientError() || starter.getStatus().hadServerError()){
                String log = starter.getStatus().toString();
                logger.error(log);
                System.err.println(log);
                System.exit(ERR_RABBITMQ);
            }
    ...
    private static void checkAndStartServer(Component component) {
        logger.trace("Starting the OpenStack Event collector");
        try {
