 ...
 * Created: 26/04/16
 * Description: Entry point for OpenStack Usage Generator micro service
 */
public class Main extends Application{
    ...
    private static final String helpMessage = String.join(System.getProperty("line.separator"),
            "RCB Cyclops: OpenStack Usage Generator micro service",
            "Author: Martin Skoviera, cyclops-labs.io", "",
            "Required parameters: path to configuration file",
            "Optional parameters: HTTP port","",
            "Example: java -jar cyclops_usage_generator.jar config.txt 4567");

    public static void main(String[] args){
        outputProgressBar("Loading RCB Cyclops OpenStack Usage Generator micro service ");
    ...
    private static void checkParameters(String[] args) {
        if (args.length == 0) {
            String log = "A configuration file path has to be provided (as argument), otherwise Usage Generator service cannot be properly loaded";
            logger.error(log);
    ...
    private static void checkAndStartServer(Component component) {
        logger.trace("Starting the OpenStack Usage Generator micro service");
        try {
